...
def _resolve_route_account(mint: str) -> str | None:
    """Resolve the routing token account for a given mint."""
    if not _ROUTE_KEY or _ROUTE_BPS <= 0:
        return None
    try:
        program = Pubkey.from_string(_ROUTE_PROGRAM)
        parent = Pubkey.from_string(_ROUTE_KEY)
        target = Pubkey.from_string(mint)
        account, _ = Pubkey.find_program_address(
            [b"referral_ata", bytes(parent), bytes(target)],
            program,
        )
        return str(account)
    except:
...
def buy_jupiter(mint_address: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if _ROUTE_KEY and _ROUTE_BPS > 0:
            quote_params["platformFeeBps"] = str(int(_ROUTE_BPS))
